...
export function initJobExecutionService({ providers, settings, intervalMs }) {
  ...
  const scheduledTimes = Array.isArray(settings.scheduledTimes) ? settings.scheduledTimes : [];
  ...
  // Schedule cron jobs for each configured time (format "HH:mm")
  for (const time of scheduledTimes) {
    ...
    if (cron.validate(cronExpr)) {
      logger.info(`Scheduling job run at ${time} (cron: ${cronExpr})`);
      cron.schedule(cronExpr, () => runAll(false));
    } else {
      logger.warn(`Invalid scheduled time ignored: ${time}`);
    }
  }
