...
import { updateJobLastRun } from '../storage/jobStorage.js';
 ...
 * @param {number} deps.intervalMs - Scheduler interval in milliseconds. If not finite or <= 0, or intervalEnabled is false, the scheduler is not started.
 * @returns {void}
 */
export function initJobExecutionService({ providers, settings, intervalMs }) {
  const intervalEnabled = settings.intervalEnabled !== false;
  ...
  // Start interval-based scheduler (only if enabled)
  if (intervalEnabled && Number.isFinite(intervalMs) && intervalMs > 0) {
    setInterval(() => runAll(true), intervalMs);
  }

  // Every minute: check current settings for scheduled times and run if matched.
  // Reading settings live means no restart needed when times are changed in the UI.
  cron.schedule('* * * * *', async () => {
    try {
      const currentSettings = await getSettings();
      const rawTimes = Array.isArray(currentSettings.scheduledTimes) ? currentSettings.scheduledTimes : [];
      // Normalize stored times to HH:mm to be safe against format inconsistencies
      ...
      logger.debug(`Scheduled time check: now=${currentTime}, configured=${JSON.stringify(times)}`);
      if (times.length > 0 && times.includes(currentTime)) {
        logger.info(`Running scheduled jobs for time ${currentTime}`);
        runAll(false);
      }
    } catch (err) {
      logger.warn('Failed to check scheduled times', err);
    }
  });
