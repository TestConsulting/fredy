...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    const findSimilarStmt = db.prepare(
      `SELECT id, additional_sources FROM listings
       WHERE job_id = @job_id
    ...
    const updateSourcesStmt = db.prepare(`UPDATE listings SET additional_sources = @sources WHERE id = @id`);

    for (const item of listings) {
      const address = removeParentheses(item.address);

      // Check if same property already exists from a different provider in this job
      const similar = findSimilarStmt.get({
        job_id: jobId,
        provider: providerId,
        title: item.title,
        price: item.price,
      ...
      if (similar) {
        // Merge new provider+link into additional_sources of the existing listing
        const existing = similar.additional_sources ? JSON.parse(similar.additional_sources) : [];
        const alreadyAdded = existing.some((s) => s.provider === providerId && s.link === item.link);
        if (!alreadyAdded) {
          existing.push({ provider: providerId, link: item.link });
          updateSourcesStmt.run({ id: similar.id, sources: JSON.stringify(existing) });
        }
        continue;
      }
 ...
 * @returns {any} The result from SqliteConnection.execute.
 */
export const deleteAllListings = (hardDelete = false) => {
  if (hardDelete) {
    return SqliteConnection.execute(`DELETE FROM listings`);
  }
  return SqliteConnection.execute(`UPDATE listings SET manually_deleted = 1`);
};
