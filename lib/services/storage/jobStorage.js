...
/**
 * Update the last_run timestamp of a job.
 * @param {string} jobId
 * @param {number} [ts] - Epoch ms; defaults to Date.now().
 */
export const updateJobLastRun = (jobId, ts = Date.now()) => {
  SqliteConnection.execute(`UPDATE jobs SET last_run = @ts WHERE id = @id`, { id: jobId, ts });
};
