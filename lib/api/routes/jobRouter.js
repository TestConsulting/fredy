...
import { isRunning as isJobRunning, getRunningJobIds } from '../../services/jobs/run-state.js';
...
jobRouter.get('/events', async (req, res) => {
  ...
  try {
    ...
    // Immediately sync current running state so reconnects don't miss events
    const runningIds = getRunningJobIds();
    for (const jobId of runningIds) {
      res.write(`event: jobStatus\n`);
      res.write(`data: ${JSON.stringify({ jobId, running: true })}\n`);
      res.write('\n');
    }
